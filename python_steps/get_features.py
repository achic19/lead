...
class GetFeatures:
    def __init__(self, network, polygon, output_name):
        ...
        # the polygons and the points will be saved hare
        self.lm_near =[]
        ...
        if isinstance(polygon, str):
            lm = ox.geometries_from_place(polygon, tags=self.dict_poi)
        else:
            lm = ox.geometries_from_polygon(polygon, tags=self.dict_poi)
        ...
        network_with_features.to_file(output_name, driver='GPKG', layer='_features')
        self.line_results = network_with_features


    def save_lm(self,lm,folder_name:str):
        # delete unnecessary rows
        for geometry_type in ['Point', 'Polygon']:
            print(' ' + geometry_type)
            lm_temp = lm[lm['geometry'].type.values == geometry_type]
            # delete unnecessary rows
            lm_temp = lm_temp[lm_temp.loc[:, lm_temp.columns.values[:-1]].any(axis=1, skipna=False, bool_only=True)]
            lm_temp.to_file(folder_name, driver='GPKG', layer=geometry_type)
            self.dict_poi[geometry_type] = lm_temp
